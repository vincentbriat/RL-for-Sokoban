...
class SokobanEnv(gym.Env):
    ...
    def __init__(self,
        ...
        self.map_filenames = ['maps/map1.txt',
                              'maps/map2.txt',
                              'maps/map3.txt',
                              'maps/map4.txt',
                              'maps/map5.txt']
    ...
    def setup_level(self, level: int):
        if level < 1:
            ...
            return
        if not level - 1 < len(self.map_filenames):
            print(f"Cannot find filepath for level {level} in map_filenames (can't find index {level - 1})")
            return
        self.level = level
        self.load_map_txt(self.map_filenames[level - 1])

    def find_player_position(self):
        return np.array(np.where(self.map == '@'))[:, 0]
    
    def find_box_positions(self):
        return np.array(np.where(np.logical_or(self.map == '$', self.map == '!')))
    ...
    def fix_targets(self):
        ...
        if tile == ' ':
            self.map[self.target_position[0], self.target_position[1]] = 'O'

    def reset(self):
        self.load_map_txt(self.map_filenames[self.level - 1])
        self.player_position = self.find_player_position()
        self.steps = 0
